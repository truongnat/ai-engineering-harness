...
const { buildInstallPlan, printPlan, warnNonGitPrivate } = require("./cli-plan");
const {
  packRootFromModule,
  runAihSh,
  buildInstallArgs,
...
function printHeader(packRoot, targetAbs) {
  ...
  console.log("");
  console.log(`Package: ai-engineering-harness@${version}`);
...
function printDetectedHints(targetAbs) {
  const recommended = detectRecommendedProviders(targetAbs);
  if (recommended.length === 0) {
    console.log("Detected providers: (none — choose manually)");
  } else {
    console.log(`Detected providers (recommended, not auto-installed): ${recommended.join(", ")}`);
  }
  console.log("");
}
