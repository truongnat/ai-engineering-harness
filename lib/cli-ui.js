...
const { SLASH_COMMANDS } = require("../runtime-command-catalog.js");
...
function showInstallPlan(plan, { compact = false } = {}) {
  const body = [
    ...
    SLASH_COMMANDS.slice(0, 4).map((s) => `  ${s}`).join("\n"),
    "  …"
  ].join("\n");
...
function showUninstallPlan(ctx, { compact = false } = {}) {
  ...
  note(body, { title: "Uninstall plan" });
}

function showWarning(message) {
  note(message, { title: "Warning" });
}
