...
function formatDoctorCommandLines(targetRoot, detectedRuntimes) {
  ...
  for (const rt of detectedRuntimes) {
    ...
    if (spec.status === "plugin-packaging" && rt === "codex") {
      ...
      if (rt === "opencode") {
        const ocPlan = path.join(targetRoot, ".opencode/commands/harness-plan.md");
        if (!fs.existsSync(ocPlan)) {
          lines.push("FAIL OpenCode native command file missing: .opencode/commands/harness-plan.md");
        }
      }
      if (rt === "claude") {
